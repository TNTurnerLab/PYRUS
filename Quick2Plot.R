...
if (!is.null(opt$file)){
  ...
  if (!is.null(opt$dir)){
    ...
    for (row in 1:nrow(c)){
      ...
      #d<- lapply(fileplot, fread(fileplot))
...
while (i <= length(chrom)){
  ...
  if (i >= 0){
    ...
    #write.table(test2, file = "CHR_TABLE.txt", row.names=TRUE)
    
    #test2 <- test %>% filter(between(chrom[[i]]$start,as.numeric(stop),as.numeric(start)))
    ...
    #test2<- test %>% filter( as.numeric(test2$stop) <= as.numeric(chrom[[i]]$stop) )
...
if (!is.null(opt$cordfile) && !is.null(opt$plotTogether) &&  is.null(opt$dir)){
  ...
  for(row in 1:nrow(newdataset)){
    ...
    stopit<- newdataset[row,"stop"]
      ...
      #exonfilt<- testz %>% select("Chr","start","stop","Ecopynum")
        ...
        while (j <= length(exondf$start )){
          for (j in 1:nrow(exondf)){
            ...
            if(exonledgend %like% "y" || exonledgend %like% "yes"){
              ...
              #plot<-plot  + aes(x=exondf$start,y =exondf$end)+ theme(legend.position = "top")
              plot<-plot1 
            }
            else{
...
if (!is.null(opt$dir) && !is.null(opt$cordfile)){
  ...
  a<-toString(rbind(str_trunc(h,20,"right")))
   ...
   if (!is.null(opt$multifile)){
    ...
    # file.rename("CHR_TABLE.txt",filename)
  for (l in list){
    file.remove(l)
  }
   }
